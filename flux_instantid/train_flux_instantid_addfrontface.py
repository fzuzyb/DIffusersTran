...
@torch.no_grad()
def log_validation(flux_transformer, args, accelerator, weight_dtype, step, pipline=None, vae=None, is_final_validation=False):
    ...
    if not is_final_validation:
        ...
        pipeline = FluxControlPipeline.from_pretrained(
            ...
            vae=vae,
            text_encoder=pipline.text_encoder,
            tokenizer=pipline.tokenizer,
            text_encoder_2=pipline.text_encoder_2,
            tokenizer_2=pipline.tokenizer_2,
        )
    else:
        ...
        attn_procs = {}
        for index, (name, value) in enumerate(transformer.attn_processors.items()):
            joint_attention_dim = transformer.config.joint_attention_dim if name.startswith(
                "transformer_blocks") and index % 2 == 0 else None
            hidden_size = transformer.config["num_attention_heads"] * transformer.config["attention_head_dim"]
            if joint_attention_dim is None:
                attn_procs[name] = FluxAttnProcessorModule2_0()
            else:
                attn_procs[name] = FluxIPAdapterJointAttnProcessor2_0(hidden_size=hidden_size,
                                                                      cross_attention_dim=joint_attention_dim)

        transformer.set_attn_processor(attn_procs)
        adapter_modules = torch.nn.ModuleList(transformer.attn_processors.values())
        transformer.encoder_hid_proj = ImageProjection(cross_attention_dim=transformer.config["joint_attention_dim"],
                                                            image_embed_dim=512,
                                                            num_image_text_embeds=4)  # set ip_adapter_proj
        logger.info("reading ip_adapter form {}".format(os.path.join(args.output_dir, f"checkpoint-{step}", "ip_adapter_model.ckpt")))
        ckpt = torch.load(os.path.join(args.output_dir, f"checkpoint-{step}", "ip_adapter_model.ckpt"))
        adapter_modules.load_state_dict(ckpt['ip_adapter_layer'], strict=True)
        transformer.encoder_hid_proj.load_state_dict(ckpt['image_projection'], strict=True)
    ...
    if args.speed_lora is not None:
        pipeline.load_lora_weights(args.speed_lora, adapter_name="speed")
        pipeline.set_adapters(["speed"], adapter_weights=[0.125])
        num_inference_steps = 8
    else:
        num_inference_steps = 20

    pipeline.to(accelerator.device)
    #pipeline.set_progress_bar_config(disable=F)
    ...
    validation_root = args.image_root
    with open(os.path.join(validation_root, 'val_filenames.txt'), 'r') as fo:
        filenames = [file.strip() for file in fo.readlines()]  # list: [file.jpg]
    if args.max_val_samples is not None:
        max_val_samples = args.max_val_samples if args.max_val_samples < len(filenames) else len(filenames)
    else:
        max_val_samples = len(filenames)
    face_id_embeds = []
    texts = []
    kp_images = []
    gt_images = []
    face_images = []
    for filename in filenames[:max_val_samples]:
        # read embed
        face_id_embed = np.load(os.path.join(validation_root, 'face_features', filename.replace('.jpg', '.npy')))
        face_id_embed = torch.from_numpy(face_id_embed)  # 1*512
        face_id_embeds.append(face_id_embed)
        # read face image and kp image to rgb
        kp_image = Image.open(os.path.join(validation_root, 'images_fivekp_concat', filename))
        kp_images.append(kp_image)
        face_image = Image.open(os.path.join(validation_root, 'faces_seg_woneck_frontalization', filename))
        face_images.append(face_image)
        gt_image = Image.open(os.path.join(validation_root, 'images', filename))
        gt_images.append(gt_image)
        # read text
        with open(os.path.join(validation_root, 'texts', filename.replace('.jpg', '.txt'))) as fo:
    ...
    local_images = []
    for validation_prompt, validation_image, embedding, face_image, gt_image in zip(texts, kp_images, face_id_embeds, face_images, gt_images):
        ...
        face_image = face_image.resize((args.resolution, args.resolution))
        with autocast_ctx:
            image = pipeline(
                prompt=validation_prompt,
                control_image=[validation_image, face_image],
                ...
                joint_attention_kwargs={'ip_adapter_image_embeds': embedding.to(accelerator.device, dtype=weight_dtype)}
            ).images[0]

            local_images.append(image_grid([validation_image, face_image, image, gt_image], 1, 4))
        image = image.resize((args.resolution, args.resolution))
        images.append(image)

        image_logs.append(
            {"validation_image": validation_image, "image": image, "gt_image": gt_image, "validation_prompt": validation_prompt}
        )
    # save image for local to see
    images = image_grid(local_images, len(face_id_embeds), 1)
    os.makedirs(os.path.join(args.output_dir, 'samples'), exist_ok=True)
    images.save(os.path.join(args.output_dir,'samples','{}.jpg'.format(step)))
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "tensorboard":
            ...
            for log in image_logs:
                ...
                formatted_images.append(wandb.Image(validation_image, caption="kp"))
                formatted_images.append(wandb.Image(gt_image, caption="gt"))

            tracker.log({tracker_key: formatted_images})
        else:
            logger.warning(f"image logging not implemented for {tracker.name}")
        if args.speed_lora is not None:
            pipeline.delete_adapters("speed")
        del pipeline
        free_memory()
        return image_logs
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="./data",
        help="The training data input root directory.",
    )
    parser.add_argument(
        "--speed_lora",
        type=str,
        default=None,
        help="The flux speed lora file.",
    )
    parser.add_argument(
        "--cache_dir",
        type=str,
...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, args):
        ...
        self.image_root_path = args.image_root
        with open(os.path.join(self.image_root_path,'train_filenames.txt'), 'r') as fo:
            self.data = [file.strip() for file in fo.readlines()]  # list: [file.jpg]

        self.transform = transforms.Compose([
            transforms.Resize(self.size, interpolation=transforms.InterpolationMode.BILINEAR),
            transforms.ToTensor(), # HWC2CHW 0-255->0-1
            transforms.Normalize([0.5], [0.5]), # 0-1 -> -1-1
        ])
        self._debug_max_len =None

    def set_len(self,max_len):
        self._debug_max_len = min(max_len, len(self.data))  # 不能超过实际数据量

    def __getitem__(self, idx):
        img_filename = self.data[idx]
        # read image
        image = self.transform(Image.open(os.path.join(self.image_root_path, 'images', img_filename)).convert("RGB"))  # TO RGB  3 CHW
        # read kp
        kp_image = self.transform(Image.open(os.path.join(self.image_root_path, 'images_fivekp_concat', img_filename)).convert("RGB"))  # TO RGB  3 CHW
        # read kp
        face_image = self.transform(Image.open(os.path.join(self.image_root_path, 'faces_seg_woneck_frontalization', img_filename)).convert(
                "RGB"))  # TO RGB  3 CHW

        # read face embedding
        face_id_embed = np.load(os.path.join(self.image_root_path, 'face_features', img_filename.replace('.jpg', '.npy')))
        face_id_embed = torch.from_numpy(face_id_embed)  # 1*512
        # read text
        with open(os.path.join(self.image_root_path, 'texts', img_filename.replace('.jpg', '.txt'))) as fo:
            text = fo.readline().strip()
    ...
    def __len__(self):
        # 返回实际长度或调试长度
        return self._debug_max_len if self._debug_max_len else len(self.data)


def collate_fn(data):
    images = torch.stack([example["image"] for example in data])
    images = images.to(memory_format=torch.contiguous_format).float()
    kp_images = torch.stack([example["kp_image"] for example in data])
    kp_images = kp_images.to(memory_format=torch.contiguous_format).float()
    face_images = torch.stack([example["face_image"] for example in data])
    face_images = face_images.to(memory_format=torch.contiguous_format).float()
    face_id_embed = torch.cat([example["face_id_embed"] for example in data], dim=0)
    captions = [example["text"] for example in data]
    return {
        "pixel_values": images,
        "conditioning_pixel_values": kp_images,
        "face_embedding": face_id_embed,
...
def main(args):
    ...
    flux_transformer.register_to_config(in_channels=initial_input_channels*3, out_channels=initial_input_channels)
    ...
    image_projection = ImageProjection(
        cross_attention_dim=flux_transformer.config["joint_attention_dim"],
        image_embed_dim=512,
        num_image_text_embeds=4,
    ).to(dtype=weight_dtype, device=accelerator.device)
    # output b*4*flux_transformer.config["joint_attention_dim"]
    attn_procs = {}
    for index, (name, value) in enumerate(flux_transformer.attn_processors.items()):
        joint_attention_dim = flux_transformer.config.joint_attention_dim if name.startswith(
            "transformer_blocks") and index % 2 == 0 else None
        hidden_size = flux_transformer.config["num_attention_heads"] * flux_transformer.config["attention_head_dim"]
        if joint_attention_dim is None:
            attn_procs[name] = FluxAttnProcessorModule2_0().to(dtype=weight_dtype, device=accelerator.device)
        else:
            attn_procs[name] = FluxIPAdapterJointAttnProcessor2_0(hidden_size=hidden_size,
                                                                  cross_attention_dim=joint_attention_dim).to(dtype=weight_dtype, device=accelerator.device)

    flux_transformer.set_attn_processor(attn_procs)
    adapter_modules = torch.nn.ModuleList(flux_transformer.attn_processors.values())
    flux_transformer.encoder_hid_proj = image_projection  # set ip_adapter_proj
            ...
            if accelerator.is_main_process:
                ...
                for model in models:
                    if isinstance(unwrap_model(model), type(unwrap_model(flux_transformer))):
                        ...
                        ip_adapter_state_dict['image_projection'] = model.encoder_hid_proj.state_dict()
                        ip_adapter_state_dict['ip_adapter_layer'] = torch.nn.ModuleList(model.attn_processors.values()).state_dict()


                    # elif isinstance(unwrap_model(model), type(unwrap_model(adapter_modules))):
                    #     model = unwrap_model(model)
                    #     ip_adapter_state_dict['ip_adapter_layer'] = model.state_dict()
                    # elif isinstance(unwrap_model(model), type(unwrap_model(image_projection))):
                    #     model = unwrap_model(model)
                    ...
                    if weights:
                        weights.pop()
                logger.info("save ip_adapter to {}".format(
                    os.path.join(output_dir, 'ip_adapter_model.ckpt')))
                torch.save(ip_adapter_state_dict, os.path.join(output_dir, 'ip_adapter_model.ckpt'))
            ...
            if not accelerator.distributed_type == DistributedType.DEEPSPEED:
                while len(models) > 0:
                    ...
                    if isinstance(model, type(unwrap_model(flux_transformer))):
                        transformer_ = model
                        logger.info("current model creating")
                    else:
                        raise ValueError(f"unexpected save model: {model.__class__}")
            else:
                logger.info("new model creating")
    ...
    params_to_opt = list(filter(lambda p: p.requires_grad, flux_transformer.parameters()))
    for k, param in flux_transformer.named_parameters():
        if param.requires_grad:
            logger.info(f"params {k} will be optimed")
    ...
    if args.max_train_samples is not None:
        train_dataset.set_len(args.max_train_samples)
    train_dataloader = torch.utils.data.DataLoader(
        ...
        pin_memory=True,
        persistent_workers=True if args.dataloader_num_workers > 0 else False  # 避免重复初始化进程
    )
    ...
    if args.resume_from_checkpoint:
        if args.resume_from_checkpoint != "latest":
            ...
            dirs = [d for d in dirs if d.startswith("checkpoint") and os.path.isdir(d)]
        ...
        if path is None:
            ...
            # ipadapter layer
            ckpt = torch.load(os.path.join(args.output_dir, f'checkpoint-{global_step}', 'ip_adapter_model.ckpt'), map_location=lambda storage, loc: storage)
            adapter_modules.load_state_dict(ckpt['ip_adapter_layer'], strict=True)
            image_projection.load_state_dict(ckpt['image_projection'], strict=True)
            del ckpt
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(flux_transformer):
                ...
                control_latents = encode_images(batch["conditioning_pixel_values"], vae.to(accelerator.device), weight_dtype)
                face_img_latents = encode_images(batch["face_images"], vae.to(accelerator.device), weight_dtype)
                control_latents = torch.cat([control_latents, face_img_latents], dim=1)
                if args.offload:
                    # offload vae to CPU.
                    vae.cpu()
                ...
                model_pred = flux_transformer(
                    ...
                    return_dict=False,
                    joint_attention_kwargs={'ip_adapter_image_embeds': batch["face_embedding"].to(accelerator.device, dtype=weight_dtype)}     # add face_embedding
                )[0]
                ...
                # # 记录优化前的参数值
                # before_optim = {k: param.data.clone() for k, param in flux_transformer.named_parameters() if
                #                 param.requires_grad}
                optimizer.step()
                # 记录优化后的参数值
                # after_optim = {k: param.data.clone() for k, param in flux_transformer.named_parameters() if
                #                param.requires_grad}
                # for k in before_optim:
                #     is_changed = not torch.allclose(before_optim[k], after_optim[k], atol=1e-7)
                #     change_status = "变化" if is_changed else "未变化"
                #     logger.info(f"参数 {k} {change_status}")
                lr_scheduler.step()
                optimizer.zero_grad()
                ...
                if accelerator.distributed_type == DistributedType.DEEPSPEED or accelerator.is_main_process:
                    ...
                    if global_step % args.validation_steps == 0:
                        log_validation(
                            ...
                            step=global_step,
                            pipline=text_encoding_pipeline,
                            vae=vae,
                        )
