...
from realdataset.real_webdataset_opt import Text2ImageDatasetLoader
...
def log_validation(flux_transformer, args, accelerator, weight_dtype, step, pipline=None, vae=None, is_final_validation=False):
    ...
    validation_root = args.val_image_root
    ...
    local_images = []
    for validation_prompt, validation_image, embedding, gt_image in zip(texts, kp_images, face_id_embeds, gt_images):
        ...
        with autocast_ctx:
            ...
            local_images.append(image_grid([validation_image, image, gt_image], 1, 3))
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="./data",
        help="The val data input root directory.",
    )
    parser.add_argument(
        "--speed_lora",
        type=str,
...
def main(args):
    ...
    # # load dataset
    dataset_loader = Text2ImageDatasetLoader(args.train_image_root, resolution=args.resolution,
                                             per_gpu_batch_size=args.train_batch_size,
                                             global_batch_size=args.train_batch_size*accelerator.num_processes,
                                             face_shape=args.resolution,
                                             num_workers=args.dataloader_num_workers,
                                             use_seg_face=False, accelerator=accelerator)
    ...
    logger.info("optimizer init success")

    # Scheduler and math around the number of training steps.
    # Check the PR https://github.com/huggingface/diffusers/pull/8312 for detailed explanation.
    if args.max_train_steps is None:
        len_train_dataloader_after_sharding = math.ceil(train_dataloader.num_batches / accelerator.num_processes)
    ...
    if args.max_train_steps is None:
        ...
        if num_training_steps_for_scheduler != args.max_train_steps * accelerator.num_processes:
            logger.warning(
                f"The length of the 'train_dataloader' after 'accelerator.prepare' ({train_dataloader.num_batches}) does not match "
    ...
    if args.resume_from_checkpoint:
        ...
        if path is None:
            ...
            # ipadapter layer
            ckpt = torch.load(os.path.join(args.resume_from_checkpoint, 'ip_adapter_model.ckpt'), map_location=lambda storage, loc: storage)
            adapter_modules.load_state_dict(ckpt['ip_adapter_layer'], strict=True)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(flux_transformer):
                ...
                # vae encode
                pixel_latents = encode_images(batch["pixel_values"].to(accelerator.device), vae.to(accelerator.device), weight_dtype)
                control_latents = encode_images(batch["conditioning_pixel_values"].to(accelerator.device), vae.to(accelerator.device), weight_dtype)
